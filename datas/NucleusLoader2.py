...
class NSDataset(data.Dataset):
    ...
    def _3D_image(self, idx):
        ...
        img_shape=np.array([144,144,60])
        dx = random.randint(0, input_shape[0]-img_shape[0])
        dy = random.randint(0, img_shape[1]-img_shape[1])
        dz = random.randint(0, img_shape[2]-img_shape[2])

        input_np = input_np[dx:dx + img_shape[0], dy:dy + img_shape[1], dz:dz + img_shape[2]]
        target_np = target_np[dx:dx + img_shape[0], dy:dy + img_shape[1], dz:dz + img_shape[2]]

        #t=int(input_np.shape[2]/2)
        #input_np=input_np[:,:,t-56:t+56]
        #target_np=target_np[:,:,t-56:t+56]

        #input_np=input_np[17:321,17:321,71:151]
        #target_np=target_np[17:321,17:321,71:151]
