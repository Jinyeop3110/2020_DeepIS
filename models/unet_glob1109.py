...
class Unet3D_glob1109(nn.Module):
    ...
    def __init__(self, feature_scale=1, trans_feature=24):
        ...
        self.final = nn.Sequential(nn.Conv3d(self.trans_feature, self.trans_feature // 2, 3, padding=1, bias=False),
                                    nn.BatchNorm3d(self.trans_feature // 2),
                                    nn.ReLU(inplace=True),
                                    nn.Dropout(.1),
                                    nn.Conv3d(self.trans_feature // 2, 3, 1),
                                    nn.Softmax(dim=1)
                                   )
