...
class Unet3D_glob(nn.Module):
    ...
    def __init__(self, feature_scale=1, trans_feature=32):
        ...
        self.ConBR0=ConBR(self.trans_feature, self.trans_feature)
        ...
        self.classifier3=self._classifier(self.trans_feature)
        self.classifier4=self._classifier(self.trans_feature)
...
#        gc_fm1 = self.ConBR1(torch.cat((fm1, gc_fm2), 1))
        ...
        return out, self.classifier3(fs3), self.classifier4(fs4)

    def _classifier(self, inplanes):
        return nn.Sequential(
            nn.Conv3d(inplanes, inplanes//2, 3, padding=1, bias=False),
            nn.BatchNorm3d(inplanes//2),
            nn.ReLU(inplace=True),
            nn.Dropout(.1),
            nn.Conv3d(inplanes//2, 3, 1),
            nn.Softmax(dim=1)
        )
