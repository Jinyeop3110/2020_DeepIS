...
class Unet3D_glob1111(nn.Module):

    def __init__(self, feature_scale=1, trans_feature=32):
        super(Unet3D_glob1111, self).__init__()
        layer_num = [2, 2, 2, 2]
        ...
        self.gcn2 = GCN(feature_num[1], self.trans_feature, k=(9, 9, 9))
        self.gcn3 = GCN(feature_num[2], self.trans_feature, k=(7, 7, 7)) #gcn_i after layer-1
        self.gcn4 = GCN(feature_num[3], self.trans_feature, k=(5, 5, 5))
        ...
        self.final = nn.Sequential(nn.Conv3d(self.trans_feature, 1, 1, padding=1, bias=False)
                                   )
